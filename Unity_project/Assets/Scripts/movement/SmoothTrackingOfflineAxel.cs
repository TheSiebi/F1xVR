...
public class SmoothTrackingOfflineAxel : MonoBehaviour
{
    ...
    string session_key = MainMenu.session_id;
    ...
    void InstantiateCars()
    {
        ...
        for (int i = 0; i < driver_numbers.Count; i++)
        {
            ...
            Vector3 position = new Vector3(listX[i].Peek(), listY[i].Peek(), -147);
    ...
    {
        TextAsset colorsFile = Resources.Load<TextAsset>($"{session_key}/colors");
    ...
    void LoadXYFromCSV(string csvText, int driverNumber)
    {
        ...
        for (int i = 1; i < lines.Length; i++) // Start from 1 to skip header
        {
            ...
            if (float.TryParse(fields[1], out float x) && float.TryParse(fields[2], out float y))
            {
